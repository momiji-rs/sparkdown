// @momiji-rs/sparkdown/full — CommonMark + every extension → HTML, as WASI-free
// WebAssembly. Same engine as @momiji-rs/sparkdown, compiled with the full
// extension set; render with `toHtml(md, options)`.
//
// The wasm module is base64-inlined (./full-wasm-inline.mjs, generated by
// build.mjs), so this package is self-contained: Node, browsers, bundlers, Deno,
// Bun, and edge runtimes, with no fetch, fs, or asset configuration.

import { WASM_BASE64 } from "./full-wasm-inline.mjs";
...
// Default: GFM + the common content extensions on; the opinionated *transforms*
// (hardWraps, headingIds, externalLinks) off. Pass an options object to override
// any flag, e.g. `toHtml(md, { externalLinks: true, frontmatter: false })`.
const ALL = {
  strikethrough: true,
  tasklist: true,
  autolink: true,
...
/** Render `markdown` to HTML. `options` selects extensions (default: see `ALL`). */
...
export function toHtmlSync(markdown, options) {
  if (!wasm) {
    throw new Error("sparkdown/full: call initSync() or await ready/init() before toHtmlSync()");
  }
