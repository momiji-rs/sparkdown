//! HTML5 named character references and the numeric C1 remap.
//!
//! `NAMED` is the full WHATWG entity set (semicolon forms only, as
//! CommonMark requires), vendored from <https://html.spec.whatwg.org/entities.json>.
//! Values are the resolved character(s) — 93 entities expand to two code points.

/// Named entities sorted by name for binary search: `(name, characters)`.
...
/// C1 (Windows-1252) numeric-reference remap: `(code point, replacement)`.
...
/// Resolve a named entity (`copy`, `Alpha`, …) to its character(s), or
/// `None` if unknown (the caller then leaves the `&` literal).
pub(crate) fn named(name: &str) -> Option<&'static str> {
    NAMED
...
mod tests {
    ...
    #[test]
    fn known_lookups() {
        assert_eq!(named("copy"), Some("\u{A9}"));
        assert_eq!(named("amp"), Some("&"));
        assert_eq!(named("ngE"), Some("\u{2267}\u{338}")); // two code points
